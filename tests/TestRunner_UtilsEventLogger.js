// =============================================================================
// FILE: TestRunner_UtilsEventLogger.js
...
import { runTests, mockElectronAPI } from './testUtils.js';
...

  {
    ...
    run: async () => {
      ...
      try {
        ...
        if (calls.length === 0) {
          return { ok: false, details: 'No call recorded — guard too strict' };
        }

        const params = calls[0].params || {};
        if ('password' in params) {
          return { ok: false, details: 'password present in params (should be blacklisted)' };
        }
        if ('token' in params) {
          return { ok: false, details: 'token present in params (should be blacklisted)' };
        }
        if (!('userId' in params)) {
          return { ok: false, details: 'userId missing (should pass through)' };
        }
...

  {
    ...
    run: async () => {
      ...
      try {
        ...
        if (calls.length === 0) {
          return { ok: false, details: 'No call recorded' };
        }
        ...
        if (missing.length > 0) {
          return { ok: false, details: `Missing fields: ${missing.join(', ')}` };
        }
        if (entry.module !== 'TaskPanel') {
          return { ok: false, details: `Wrong module: ${entry.module}` };
        }
        if (entry.source !== 'user') {
          return { ok: false, details: `Wrong source: ${entry.source}` };
        }
        if (typeof entry.ts !== 'number') {
          return { ok: false, details: `ts not a number: ${entry.ts}` };
        }
