// =============================================================================
// FILE: TestRunner_Limits.js
// PATH: tests/TestRunner_Limits.js
// VERSION: 0.0.3
// PURPOSE: Testy modułu limitów aplikacji (src/config/limits.js) — LIMITS, getLimit.
...
const tests = [
  {
    ...
    run: async () => {
      const { LIMITS } = await import(join(ROOT, 'src/config/limits.js'));
  ...
  },
  {
    ...
    run: async () => {
      const { getLimit, LIMITS } = await import(join(ROOT, 'src/config/limits.js'));
  ...
  },
  {
    ...
    run: async () => {
      const { getLimit } = await import(join(ROOT, 'src/config/limits.js'));
