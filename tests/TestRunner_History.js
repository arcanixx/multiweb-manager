...
const tests = [
  ...
  // env:'react' – historyStore może mieć zależność od electron (main process)
  {
  ...
  },
  {
    ...
    run: async () => {
      ...
      const ok = !validLevels.includes('critical');
      return { ok, details: ok ? '' : 'Invalid level should not be accepted' };
    }
  },

  // ── Filtrowanie (Node-safe) ────────────────────────────────────────────────
  {
