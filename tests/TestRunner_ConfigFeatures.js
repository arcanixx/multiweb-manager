// =============================================================================
// FILE: TestRunner_ConfigFeatures.js
// PATH: tests/TestRunner_ConfigFeatures.js
// VERSION: 0.0.3
// PURPOSE: Testy modułu feature flags (src/config/featuresConfig.js) — isFeatureEnabled, isToolEnabled, spójność FEATURES.
...
const tests = [
  {
    ...
    run: async () => {
      const { isFeatureEnabled } = await safeImport('src/config/featuresConfig.js');
  ...
  },
  {
    ...
    run: async () => {
      const { isFeatureEnabled, isToolEnabled } = await safeImport('src/config/featuresConfig.js');
  ...
  },
  {
    ...
    run: async () => {
      const { FEATURES } = await safeImport('src/config/featuresConfig.js');
